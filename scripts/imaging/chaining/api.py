...
Once search 2 is running, you should checkout its `model.info` file. The parameters do not use the default parameter 
priors. Instead, they use `GaussianPrior`s where:
 ...
 specified in the `width_modifier` field of the `Profile`'s entry in the `priors` `.json` files.

The `width_modifier` is used instead of the errors computed from search 1 when the errors values estimated are smaller 
than the width modifier value. This ensure that the sigma values used for priors in search 2 do not assume extremely 
small values (e.g. a value of < 0.01 for an einstein_radius) if the error estimates in search 1 are very small, which
may occur when using fast non-linear search settings or fitting an overly simplified model.
    
Thus, search 2 used the results of search 1 to inform where to search non-linear parameter space! 

The `PriorPasser` customizes how priors are passed from search 1 as follows:

 - sigma: The sigma value that the errors passed to use as the sigma values in search 1 are estimated at.
 - use_widths: If False, the "width_modifier" values in the json_prior configs are not used to override a passed
 error value.
 - use_errors: If False, errors are not passed from search 1 to set up the priors and only the "width" modifier
 entries in the configs are used.  

For the interested reader a complete description of prior passing is given in chapter 3 of **HowToLens**. Below is an 
extract of this description.
...
By invoking the `model` attribute, the prior is passed following 3 rules:
 ...
 (ii) The value specified for the profile in the `config/priors/*.json` config file`s `width_modifer` field (check 
 these files out now).
 ...
 searches - fast non-linear search settings, simplified lens models, etc.

 Therefore, the `width_modifier` in the json config files are our fallback. If the error on a parameter is suspiciously 
 small, we instead use the value specified in the widths file. These values are chosen based on our experience as being 
 a good balance broadly sampling parameter space but not being so narrow important solutions are missed. 
