...
the complexity of the model whilst exploiting prior chaining to keep the run times fast. This will be the main topic of
...
last search, we'll fit the complete, unbutchered data-set, so yeah it might take a while to run, but at that point
we've tuned our lens model priors so much the search should still run reasonably fast.
...
you might write a pipeline to perform the following analyses:
 ...
 lens galaxy's light and then discard it in every search afterwards?
