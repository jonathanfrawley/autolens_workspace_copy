...
Once a search has completed running, it results a `Result` object, which in the previous tutorials we used to plot
...
Alternatively, we can return the results from the search.run() method and manipulate them in a Python script.  
...
location `autolens_workspace/advanced`aggregator`. Here, you'll learn how to:
 ...
 - Use the aggregator to filter out results given a search name or input string.
