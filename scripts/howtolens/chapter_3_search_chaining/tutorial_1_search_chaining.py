...
Once the above search is running, you should checkout its `model.info` file. The parameters do not use the default 
...
To do this, we have to use the `Model` object in AutoFit, which allows us to turn light and mass profiles into 
`model components` whose parameters have priors that can be manipulated in an analogous fashion to to `Model`.
In fact, the individual components of the `Model` class have been `Model`'s all along! 
"""
print(lens.bulge)
print(lens.mass)
print(source.bulge)
...
"""
We can thus set up the `Model`  we desire, by first creating the individual `Model`'s of each
component and then passing the priors of each individual parameter. 

__LENS LIGHT PRIORS:__
"""
sersic = af.Model(al.lp.EllipticalSersic)

sersic.elliptical_comps.elliptical_comps = (
    result_1.model.galaxies.lens.bulge.elliptical_comps
)
sersic.intensity = result_1.model.galaxies.lens.bulge.intensity
sersic.effective_radius = result_1.model.galaxies.lens.bulge.effective_radius
...
lens.mass.elliptical_comps.elliptical_comps = (
    result_1.model.galaxies.lens.mass.elliptical_comps
)
lens.mass.einstein_radius = result_1.model.galaxies.lens.mass.einstein_radius

lens = af.Model(al.Galaxy, redshift=0.5, bulge=sersic, mass=mass)

"""
We now create and run the search, using the lens `Model` we created above.
"""
phase2_pass = al.PhaseImaging(
    search=af.DynestyStatic(
        path_prefix="howtolens",
    ...
    settings=settings,
    galaxies=af.Collection(lens=lens, source=result_1.model.galaxies.source),
)
